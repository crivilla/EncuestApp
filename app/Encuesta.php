...
ALID_STATUS = 2;
    ...
    public static function run($uuid, $user_id) {
        ...
        if(!$encuesta):
            return Encuesta::ERR_RUN_SURVEY_NOT_FOUND;
        elseif($encuesta->status !== 'draft'):
            return Encuesta::ERR_RUN_SURVEY_INVALID_STATUS;
        elseif($encuesta->status === 'ready'):
            return Encuesta::ERR_RUN_SURVEY_ALREADY_RUNNING;
        endif;
        $encuesta->status = 'ready';
        $encuesta->save();
    ...
    public static function pause($uuid, $user_id) {
        $encuesta = Encuesta::getByOwner($uuid, $user_id);
        if(!$encuesta):
            return Encuesta::ERR_PAUSE_SURVEY_NOT_FOUND;
        elseif($encuesta->status !== 'ready'):
            return Encuesta::ERR_PAUSE_SURVEY_INVALID_STATUS;
        elseif($encuesta->status === 'draft'):
            return Encuesta::ERR_PAUSE_SURVEY_ALREADY_PAUSED;
        endif;
